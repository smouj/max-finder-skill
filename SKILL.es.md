---
name: max-finder
description: Motor de descubrimiento y síntesis de investigación impulsado por IA para investigación académica y de mercado
version: 2.4.1
...
Max Finder es un motor de descubrimiento y síntesis de investigación impulsado por IA diseñado para automatizar y mejorar los flujos de trabajo de investigación académica y de mercado. Combina múltiples APIs de búsqueda, búsqueda semántica por embeddings y síntesis con LLM para encontrar, analizar y organizar materiales de investigación a escala.

### Casos de Uso Reales

1. **Revisiones Sistemáticas de Literatura**: Buscar automáticamente en arXiv, PubMed, Google Scholar y CrossRef artículos que coincidan con criterios específicos, extraer hallazgos clave, metodologías y generar tablas resumidas.

2. **Monitoreo de Inteligencia Competitiva**: Configurar monitoreo diario de fuentes de noticias, blogs tecnológicos y patentes para tecnologías o empresas específicas, con análisis de tendencias generado por IA.

3. **Investigación para Propuestas de Subvención**: Identificar rápidamente trabajos previos relevantes, estudios conflictivos y brechas de investigación para solicitudes de subvención con resúmenes listos para citas.

4. **Mapeo de Panorama Tecnológico**: Construir mapas completos de stacks tecnológicos, frameworks o metodologías analizando artículos, documentación y discusiones en foros.

5. **Búsqueda de Expertos**: Identificar y clasificar investigadores, instituciones o empresas que trabajan en topics específicos usando similitud de embeddings a través de historiales de publicaciones.

6. **Detección de Brechas de Investigación**: Analizar literatura existente para identificar áreas poco exploradas, hallazgos contradictorios o limitaciones metodológicas.

7. **Síntesis de Investigación Multi-Modal**: Extraer y correlacionar información de artículos, presentaciones, datasets y repositorios de código en grafos de conocimiento unificados.

8. **Meta-Análisis Automatizado**: Agregar hallazgos estadísticos across múltiples estudios para calcular tamaños de efecto e identificar patrones.
...
- Académico: arXiv, PubMed, IEEE Xplore, ACM Digital Library, SpringerLink, Wiley, ScienceDirect, bioRxiv, medRxiv
- Preprint: arXiv, bioRxiv, medRxiv, SSRN
- Literatura Gris: Google Scholar, Semantic Scholar, Microsoft Academic (via API), CORE
- Patentes: Google Patents, USPTO, EPO
- Noticias/Medios: Google News, NewsAPI, RSS feeds
- Código: GitHub, GitLab, arXiv源码链接
- Datasets: Kaggle, HuggingFace, Zenodo, Figshare
- Tesis: ProQuest, repositorios institucionales
- Ensayos Clínicos: ClinicalTrials.gov, EU Clinical Trials Register
...
1. **Nunca alucinar**: Si la fuente no expresa algo explícitamente, marcarlo como "unclear" o "not found". No inferir más allá del texto proporcionado.

2. **Preservar procedencia**: Cada claim sintetizado debe ser trazable a chunks específicos con paper IDs y chunk indices. Guardar mapeo de fuentes en metadata de salida.

3. **Respetar límites de tasa**: Implementar retroceso exponencial para todas las APIs. No saturar servidores. Cachear agresivamente. Delays por defecto: arXiv (3s), PubMed (0.34s), Google Scholar (60s+), Crossref (1s).

4. **Manejar muros de pago con elegancia**: No intentarbypasar DRM. Si acceso abierto no disponible, usar metadata y abstract únicamente. No loguear ni compartir contenido con derechos de autor sin permiso.

5. **Citar apropiadamente**: Siempre incluir DOIs cuando estén disponibles. Usar estilo de cita configurado exactamente. No inventar citas.

6. **Protección de privacidad**: Nunca almacenar datos personales de afiliaciones de autores (emails individuales, números de teléfono). Anonimizar en logs a menos que sea explícitamente necesario para búsqueda de expertos.

7. **Gestión de cuotas**: Rastrear uso de APIs por proveedor. Alertar si se acercan límites. Límites mensuales por defecto: OpenAI ($20), Semantic Scholar (100/day), arXiv (ilimitado pero respetuoso).

8. **Pinning de versiones**: Bloquear versiones de dependencias en `~/.maxfinder/frozen_requirements.txt`. No auto-actualizar sin comando `maxfinder upgrade` explícito.

9. **Invalidación de caché**: Usar hashing basado en contenido (SHA256) para PDFs. Re-embed solo cuando el contenido cambie. Cache TTL respeta `MAX_FINDER_CACHE_TTL`.

10. **Fail open**: Si cualquier fuente individual falla, continuar con las otras. Loguear errores pero no abortar búsqueda completa. Solo fallar si todas las fuentes no disponibles.
...
[2024-03-16 09:05:20] Synthesis complete. Key findings:
